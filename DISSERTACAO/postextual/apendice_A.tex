...
\section{Formulação Preditor-Corretor e Newton-Raphson}

Devido às não linearidades das forças de engrenamento, a solução implícita direta não é possível, adotando-se uma abordagem preditor-corretor com Newton-Raphson. Na etapa de \textbf{previsão}, as estimativas para o próximo passo assumem um incremento nulo de aceleração:
...
Transições abruptas por choques e perda de contato nas engrenagens exigem controle rigoroso do incremento de tempo. Para otimizar o custo computacional mantendo a estabilidade, utiliza-se um passo de tempo adaptativo baseado nas iterações do Newton-Raphson:

\begin{itemize}
	\item \textbf{Convergência Rápida:} Se atingida em poucas iterações (e.g., $\le 5$), o subpasso ($\Delta t_{sub}$) é duplicado, respeitando o limite do passo macro.
	\item \textbf{Convergência Lenta:} Se demandar muitas iterações (e.g., $\ge 10$), o subpasso é reduzido à metade, com um limite mínimo para evitar interrupções.
	\item \textbf{Divergência:} Caso não atinja a tolerância em 15 iterações, o passo é descartado, o subpasso cai para $25\%$ de seu valor e a integração retrocede.
\end{itemize}

A rotina foi implementada em Python e compilada \textit{just-in-time} (JIT) via \texttt{Numba}, garantindo desempenho computacional análogo a linguagens de baixo nível.