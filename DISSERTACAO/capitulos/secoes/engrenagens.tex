...
\subsection{Geometria e Projeto de Engrenagens}

O dimensionamento geométrico e cinemático de um par engrenado constitui a base para o desenvolvimento de modelos dinâmicos e vibratórios precisos. A principal premissa no projeto de engrenagens cilíndricas é a garantia de uma relação de transmissão de velocidade angular estritamente constante, princípio regido pela teoria do engrenamento conjugado \cite{norton2013}. Para assegurar que essa condição seja atendida mesmo diante de pequenas variações na distância entre eixos, adota-se universalmente o perfil de dente baseado na evolvente de círculo \cite{shigley2011}.

A macrogeometria de um par engrenado — composto pelo pinhão (elemento motriz, índice 1) e pela coroa (elemento movido, índice 2) — é definida por um conjunto de parâmetros primários normatizados de fabricação. Estes incluem o número de dentes ($Z_1$ e $Z_2$), o módulo normal ($m_n$), o ângulo de pressão normal ($\alpha_n$) e o ângulo de hélice ($\beta_h$). Em engrenagens helicoidais, as propriedades geométricas devem ser projetadas do plano normal (plano gerador do dente) para o plano transversal (plano de rotação), de acordo com as relações mecânicas estabelecidas por \citeonline{shigley2011}:
\begin{equation}
