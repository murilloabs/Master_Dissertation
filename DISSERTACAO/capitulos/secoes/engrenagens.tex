...
$Z_1$ representa o número de dentes da engrenagem menor, $\Omega$ é a velocidade de rotação da engrenagem menor, $c_r$ é a razão de contato e $\beta$ é o angulo de posição das engrenagens. $a^{(s)}$ e $b^{(s)}$ são os coeficientes da série de Fourier. A Figura \ref{fig:rig_engr_kapl} apresenta o perfil retangular desejado para essa simplificação, já a Figura \ref{fig:rig_engr1} apresenta a composição completa da Série de Fourier para essa função. Quando maior o número de temos $s$, mais suave é a função. Pode se observar como o valor da razão de contato $c_r$ influencia no período da função de rigidez. Esta variável indica a quantidade média de pares de dentes engrenados, ao longo do tempo há a alternância  entre 1 par e 2 pares de dentes engrenados. Quando há somente 1 par o perfil de rigidez encontra seu valor mínimo, já com 2 pares ele encontra seu valor máximo.
